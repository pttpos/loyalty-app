...
const HorizontalBannerAdminScreen = () => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      const bannersSnapshot = await getDocs(collection(db, 'banner_horizontal'));
  ...
  const handleImageUpload = async (uri: string) => {
    ...
    const storageRef = ref(storage, `banner_horizontal/${filename}`);
  ...
  const confirmBannerUpload = async () => {
    try {
      ...
      const bannerRef = doc(collection(db, 'banner_horizontal'));
  ...
  const deleteBanner = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'banner_horizontal', bannerToDelete.id));
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Admin - Manage Horizontal Banners</Text>
      <FlatList
        data={banners}
...
export default HorizontalBannerAdminScreen;
